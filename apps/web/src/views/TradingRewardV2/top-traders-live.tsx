...
interface LiveCompetition {
  ...
  const { data, isLoading } = useSWR<LiveCompetition>(
    `${COMPETITION_V2_API_URL}/live-competitions?chainId=${chainId}&page=${currentPage}&keyword=${keyword}&compType=${competitionType}&sortBy=${sortBy}&reverseSort=${reverseSort}`
    , fetcher
  )
