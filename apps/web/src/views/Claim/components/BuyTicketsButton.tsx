...
const BuyTicketsButton: React.FC<React.PropsWithChildren<BuyTicketsButtonProps>> = ({
  ...
  const { data: rewardCalculatedData, write: calculate } = useContractWrite({
    ...config,
  })
  ...
  const getLastReward = async () => {
    const fetchedLastReward = await client.readContract({
      address: contract.address as `0x${string}`,
      abi: claimADAbi,
  ...
  const [openWinRateModal] = useModal(<WinRateModal reward={reward} />)
