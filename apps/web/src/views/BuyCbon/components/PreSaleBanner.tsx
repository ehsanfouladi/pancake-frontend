      ...
      <S.Inner>
        ...
        >
          {/* <Flex flexDirection={['column', 'row']} mb={['8px', '8px', '12px']}>
           ...
           <Flex flexDirection={['column', 'row']} mb={['8px', '8px', '12px']}>
          <Title>{t(' Cbon presale sold out!')}</Title>
          </Flex>
          <Flex mt='12px'>
          <StyledBox>{t('We are now accepting token presale projects. If you want to presale your token, reach us.')}</StyledBox>
          </Flex>
          <Flex style={{ gap: 8 }} flexWrap={isMobile ? 'wrap' : 'nowrap'}>
            <NextLinkFromReactRouter to="https://t.me/cadinuLover">
              <StyledButtonLeft scale={isMobile ? 'sm' : 'md'}>
                <Text bold fontSize="16px" mr="4px">
                  {`${t('Contact us')} ${isMobile ? '👈' : ''}`}
                </Text>
