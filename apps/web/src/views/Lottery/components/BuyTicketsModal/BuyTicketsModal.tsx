...
import { usePriceCadinuUSD, usePriceCakeUSD } from 'state/farms/hooks'
...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({ onDismiss }) => {
  ...
  const cadinuPriceBusd = usePriceCadinuUSD()
  ...
  const validateInput = useCallback(
    (inputNumber: BigNumber) => {
      ...
      if (cakeCostAfterDiscount.gt(userCadinu)) {
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCadinu) return t('Insufficient CADINU balance')
  ...
  const disableBuying = useMemo(
    ...
    [isConfirming, userNotEnoughCadinu, ticketsToBuy, getTicketsForPurchase],
  )
    ...
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          ...
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInCadinu.times(new BigNumber(ticketsToBuy))) : '0.00'} CADINU`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCadinu && getFullDisplayBalance(priceTicketInCadinu.times(ticketsToBuy || 0))} CADINU
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
