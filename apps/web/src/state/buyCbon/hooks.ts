...
import { BuyCbonState, buyCbonReducerAtom } from 'state/buyCbon/reducer'
...
import { SUPPORTED_ONRAMP_TOKENS } from 'views/BuyCbon/constants'
import { Field, replaceBuyCbonState, selectCurrency, setMinAmount, setUsersIpAddress, typeInput } from './actions'
...
export function useBuyCbonState() {
  return useAtomValue(buyCbonReducerAtom)
}
...
export function useBuyCbonErrorInfo(
  typedValue: string,
  minAmount: number,
...
export function useBuyCbonActionHandlers(): {
  ...
  const [, dispatch] = useAtom(buyCbonReducerAtom)
...
const DEFAULT_FIAT_CURRENCY = 'BNB'

export async function queryParametersToBuyCbonState(
  parsedQs: ParsedUrlQuery,
  account: string | undefined,
  chainId: number,
): Promise<BuyCbonState> {
...
export function useDefaultsFromURLSearch(account: string | undefined) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const parsed = await queryParametersToBuyCbonState(query, account, chainId)
